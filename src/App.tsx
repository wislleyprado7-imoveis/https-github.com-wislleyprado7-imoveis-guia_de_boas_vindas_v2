  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        sbGuests = guestsResult.data;
        gError = guestsResult.error;

        // Fallback: If phone column is missing, load without it
        if (gError && (gError.message?.includes("phone") || gError.code === "42703")) {
          console.warn("A coluna 'phone' nao existe no banco remoto. Carregando hospedes sem ela...");
          const fallbackResult = await supabase
            .from("guests")
            .select("id, name, slug, ranch_id, check_in_date, check_out_date, is_always_unlocked, created_at");
          
          if (!fallbackResult.error) {
            sbGuests = fallbackResult.data;
            gError = null;
          } else {
            gError = fallbackResult.error;
          }
        }
